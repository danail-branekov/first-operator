...
import (
	...
	appv1alpha1 "first-operator/first-operator/pkg/apis/app/v1alpha1"
...
func (r *ReconcilePodSet) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	status := appv1alpha1.PodSetStatus{
		...
		Reason:   fmt.Sprintf("Pods:%d", len(existingPodNames)),
	}
	if !reflect.DeepEqual(podSet.Status, status) {
		podSet.Status = status
		err := r.client.Update(context.TODO(), podSet)
		if err != nil {
			reqLogger.Error(err, "failed to update the podSet status")
			return reconcile.Result{}, err
		}
	}

	// Update the foo if necessary
	foo := appv1alpha1.PodSetFoo{
		Foo: "bar",
	}
	if !reflect.DeepEqual(podSet.Foo, foo) {
		podSet.Foo = foo
		err := r.client.Update(context.TODO(), podSet)
		if err != nil {
			reqLogger.Error(err, "failed to update the podSet foo")
			return reconcile.Result{}, err
		}
	}
